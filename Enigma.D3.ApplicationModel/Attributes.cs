...
namespace Enigma.D3.ApplicationModel
{
    ...
    {
        public static double GetAttributeValue(this ACD acd, AttributeId attribId, int modifier = -1)
        {
            var attribDef = MemoryContext.Current.DataSegment.AttributeDescriptors[(int)attribId];
            if (attribDef == null)
                throw new ArgumentOutOfRangeException("attribId");

            int value;
            if (!TryGetAttributeValue(acd, attribId, modifier, out value))
                return Int32OrSingleToDouble(attribDef.DefaultValue, attribDef.DataType);

            var union = new AttributeValue { Int32 = value };
            return attribDef.DataType == typeof(int) ? union.Int32 : union.Single;
        }

        public static IEnumerable<KeyValuePair<int, AttributeValue>> EnumerateAttributes(this ACD acd)
        {
            var groupId = acd.FastAttribGroupID;
            var group = MemoryContext.Current.DataSegment.ObjectManager.FastAttrib.FastAttribGroups[(short)groupId];
            if (group != null)
            {
                var smallMap = group.PtrMap.Dereference();
                if ((group.Flags & 4) != 0 && smallMap != null)
                {
                    foreach (var bucket in smallMap.Buckets)
                    {
                        var immutableBucket = bucket.Dereference();
                        while (immutableBucket != null)
                        {
                            immutableBucket.TakeSnapshot();
                            yield return new KeyValuePair<int, AttributeValue>(
                                immutableBucket.Key,
                                new AttributeValue { Int32 = immutableBucket.Value });
                            immutableBucket = immutableBucket.GetNext();
                        }
                    }
                }
                var map = group.Map;
        ...
        internal static bool TryGetAttributeValue(this ACD acd, AttributeId attribId, int modifier, out int value)
        {
            ...
            if (group != null)
            {
                if (((group.Flags & 4) != 0 && group.PtrMap.Dereference().TryGetValue(key, out value, GetHashOfKey)) ||
        ...
        private static double Int32OrSingleToDouble(int value, Type type)
        {
            return type == typeof(int) ? value : BitConverter.ToSingle(BitConverter.GetBytes(value), 0);
        }
    }

    public abstract class Attribute<T> where T : struct
    {
        public T GetValue(ACD acd)
        {
        ...
        public abstract T GetValue(ACD acd, int modifier);
    }
    ...
    public static partial class Attributes
    {
        ...
        internal class SimpleAttribute<T> : Attribute<T> where T : struct
        {
            ...
            public override T GetValue(ACD acd, int modifier = -1)
            {
                int value;
                ...
                var union = new AttributeValue { Int32 = value };
                return (T)(_isInteger ? (object)union.Int32 : (object)union.Single);
            }
        }
    }
